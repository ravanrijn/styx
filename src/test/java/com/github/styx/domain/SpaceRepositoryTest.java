...
public class SpaceRepositoryTest {
    ...
    public void shouldFailWhenSpaceIsNotDeleted() throws IOException {
        ...
        when(restTemplate.exchange(eq("/api/v2/spaces/123"), eq(HttpMethod.DELETE), isA(HttpEntity.class), eq(String.class))).thenReturn(new ResponseEntity(HttpStatus.NO_CONTENT));
    ...
    public void shouldFailWhenSpaceIsNotReturned() throws IOException {
        ...
        when(restTemplate.exchange(eq("/api/v2/spaces/123?inline-relations-depth=2"), eq(HttpMethod.GET), isA(HttpEntity.class), eq(String.class))).thenReturn(new ResponseEntity(spaceResponse, HttpStatus.OK));
        when(restTemplate.exchange(eq("/uaa/ids/Users?filter=id eq 'f939a538-c0f1-48ef-90bc-8fd2c9ce477e'"), eq(HttpMethod.GET), isA(HttpEntity.class), eq(String.class))).thenReturn(new ResponseEntity(usersResponse, HttpStatus.OK));
